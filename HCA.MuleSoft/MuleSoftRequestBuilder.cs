...
public class MuleSoftRequestBuilder : IMuleSoftRequestBuilder
{
    ...
    private Identity BuildIdentity(DemographicSearchClientIdentityRequest demographicSearchRequest)
    {
        ...
        {
            identity.Names.Add(new(name.First, name.Middle, name.Last, name.Suffix));
        }

        foreach (var address in request.Addresses)
        {
            identity.Addresses.Add(new(address.Line1, address.Line2, address.City, address.State, address.PostalCode));
        }

        foreach (var phoneNumber in request.PhoneNumbers)
        {
            identity.PhoneNumbers.Add(new(phoneNumber.Number, phoneNumber.AreaCode, phoneNumber.Extension, phoneNumber.CountryCode));
        }
