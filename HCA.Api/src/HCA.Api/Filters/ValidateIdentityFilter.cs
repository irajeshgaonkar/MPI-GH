...
namespace HCA.Api.Filters {
    public class ValidateIdentityFilter( IAppLogger logger ) : IAsyncActionFilter {
        ...
        private static readonly Dictionary<string, int> basicAttributesToLengths = new()
        {
            ...
            ["content.identity.sources.name"] = 50,
            ["content.identity.sources.id"] = 50,
            ["content.identity.names.first"] = 40,
            ["content.identity.names.middle"] = 40,
            ["content.identity.names.last"] = 40,
            ["content.identity.names.suffix"] = 40,
